...
예) 매장, 포장 버튼이 있다면 "어르신, 매장에서 드실 건가요, 아니면 포장해 가실 건가요?" 처럼 질문을 만드세요.
    버튼에 메뉴 4개 이상이라면, 매뉴를 선택하는 화면입니다. **메뉴 선택 화면에서는 메뉴 데이터베이스의 계층 구조를 최우선으로 따라서 선택지를 제시해주세요**
...
- 어르신께서 찾으시는 메뉴가 현재 화면의 버튼에 없지만, 메뉴 데이터베이스 상 해당 카테고리 하위 항목에 존재하고, 스크롤이 가능한 화면이라면:
  - `matched_button`,`follow_up_question`과 `choices`를 모두 비워두세요.
...
7. matched_button 값은 반드시 visible_buttons 배열에 있는 버튼의 text와 일치해야 합니다
