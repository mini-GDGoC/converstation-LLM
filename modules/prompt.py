...
- 버튼 구성에 따라 질문 생성
    ...
    세트 선택 -> 사이드/음료 버튼이 있을 경우 → 메뉴 데이터의 해당 카테고리 기준으로 계층에 따라 친절히 추천
- 추천 메뉴 버튼은 제외
- matched_button: null
- follow_up_question: 버튼 기반 질문
...
# 계층 탐색 예시
- "햄버거" 언급 → 상위 카테고리 매칭 → 하위 분기 제시 ("소고기, 닭고기, 새우")
    → "소고기" 선택 시 → 다시 하위 분기 제시 ("고기 두 장, 달달한 소스" 등)
